...
<html lang="en">
    <head>
        ...
        <title>Xóa khóa đào tạo - HRM System</title>
                        ...
                        <div class="mt-4 w-50 mx-auto">
                            <div class="card border-danger">
                                <div class="card-header bg-danger text-white">
                                    <h5 class="mb-0">
                                        <i class="fas fa-exclamation-triangle me-2"></i>Xác nhận xóa khóa đào tạo
                                    </h5>
                                </div>
                                <div class="card-body">
                                    <div class="alert alert-warning">
                                        <h6>
                                            <i class="fas fa-warning me-2"></i>Bạn có chắc chắn muốn xóa khóa đào tạo
                                            này?
                                        </h6>
                                        <hr />
                                        <p><strong>ID:</strong> ${training.id}</p>
                                        <p><strong>Tên khóa học:</strong> ${training.title}</p>
                                        <c:if test="${not empty training.startDate}">
                                            <p><strong>Ngày bắt đầu:</strong> ${training.startDate}</p>
                                        </c:if>
                                        <c:if test="${not empty training.endDate}">
                                            <p><strong>Ngày kết thúc:</strong> ${training.endDate}</p>
                                        </c:if>
                                        <c:if test="${not empty employeeTrainings}">
                                            <p>
                                                <strong>Số nhân viên tham gia:</strong>
                                                ${employeeTrainings.size()}
                                            </p>
                                        </c:if>
                                        <p class="mb-0 text-danger">
                                            <small
                                                ><i class="fas fa-info-circle me-1"></i>Hành động này không thể hoàn
                                                tác! Tất cả dữ liệu liên quan đến khóa đào tạo sẽ bị xóa.</small
                                            >
                                        </p>
                                    </div>

                                    <div class="d-flex justify-content-between align-items-center">
                                        <form method="post" action="/admin/training/delete/${training.id}">
